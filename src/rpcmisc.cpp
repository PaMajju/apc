        ...
        if (mine != ISMINE_NO) {
          ...
          txnouttype whichType;
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CCarebitcoinAddress(addr).ToString());
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <Carebitaddress>\n"
            "Return information about <Carebitaddress>.");

    CCarebitcoinAddress address(params[0].get_str());
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <Carebitpubkey>\n"
            "Return information about <Carebitpubkey>.");
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <Carebitaddress> <signature> <message>\n"
    ...
    CCarebitcoinAddress addr(strAddress);
    if (!addr.IsValid())
